...
- **Portainer Registry Configuration**: If the upstream container depends on private images (e.g., from `ghcr.io`), local `docker login` over SSH is not available. Hooks must use the Portainer API to register the container registry credentials directly in Portainer (e.g., `POST /api/registries`) before creating the stack.
- **Image Pre-Pulling Abstraction**: Because SSH `docker pull` is bypassed, any required sidecar images must be pulled by querying the Portainer endpoints API (e.g., `POST /api/endpoints/{id}/docker/images/create`). Hooks should abstract this REST call to force-pull app-specific sidecars via Portainer before deployment.
