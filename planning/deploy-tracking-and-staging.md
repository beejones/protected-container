...
## Follow-up: Rename `ubuntu_deploy.py` → `portainer_deploy.py` (Phase 7)

### Rationale
The script has zero Ubuntu-specific dependencies (no apt, systemctl, etc.). It relies on SSH + Portainer + Docker Compose, which work on any Linux host. The "ubuntu" naming is historical and misleading. Rename to `portainer_deploy.py` to reflect the actual deployment mechanism.

### Scope
- [ ] Rename `scripts/deploy/ubuntu_deploy.py` → `scripts/deploy/portainer_deploy.py`
- [ ] Rename env keys: `UBUNTU_SSH_HOST` → `SSH_HOST`, `UBUNTU_REMOTE_DIR` → `REMOTE_DIR`, `UBUNTU_COMPOSE_FILES` → `COMPOSE_FILES`, `UBUNTU_BUILD_PUSH` → `BUILD_PUSH`, `UBUNTU_SYNC_SECRETS` → `SYNC_SECRETS`
- [ ] Update `env_schema.py` with new key names (keep old names as deprecated aliases)
- [ ] Update `env.deploy.example` with new key names
- [ ] Rename `docker/docker-compose.ubuntu.yml` → `docker/docker-compose.host.yml`
- [ ] Update all docs: `UBUNTU_SERVER.md` → `HOST_DEPLOY.md` (or `PORTAINER_DEPLOY.md`)
- [ ] Update `docs/deploy/STAGING.md` references
- [ ] Update GitHub Actions workflows referencing the old script name
- [ ] Update `AGENT.md` and `AGENT_APP_SPECIFIC.md`
- [ ] Keep a thin `ubuntu_deploy.py` wrapper that imports and calls `portainer_deploy.main()` with a deprecation warning (for backwards compat)
- [ ] Update tests
