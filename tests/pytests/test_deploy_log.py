...
class TestAppendDeployRecord:
    ...
    def test_production_success_for_new_git_ref_records_current_version(self, tmp_repo: Path) -> None:
        csv_path = append_deploy_record(
            repo_root=tmp_repo,
            target="production",
    ...
    def test_success_deploy_for_new_git_ref_records_current_version(self, tmp_repo: Path) -> None:
        csv_path = append_deploy_record(
            repo_root=tmp_repo,
            target="production",
            stack_name="prod-stack",
            ...
            status="success",
            git_ref="d" * 40,
            local_branch="feature/oidc-test",
        )

        rows = list(csv.reader(csv_path.open()))
        assert rows[1][1] == "d" * 40
        assert rows[1][2] == "feature/oidc-test"
        assert rows[1][3] == "1.2.3"
        assert rows[1][4] == "production"
    ...
    def test_staging_success_for_new_git_ref_records_current_version(self, tmp_repo: Path) -> None:
        _write_app_version(tmp_repo, "1.2.4")
        (tmp_repo / "CHANGELOG.md").write_text("# Changelog\n\n## [1.2.4] - 2026-06-09\n")
        csv_path = tmp_repo / "out" / "deploy" / "version_log.csv"
        csv_path.parent.mkdir(parents=True)
