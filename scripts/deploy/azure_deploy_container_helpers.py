...
DEPLOY_ENV_MATERIALIZE_KEYS: list[str] = [
    spec.key.value for spec in DEPLOY_SCHEMA if EnvTarget.DOTENV_DEPLOY in spec.targets
]
...
def materialize_deploy_env_file_if_missing(*, path: Path) -> None:
    ...
    lines: list[str] = [
        "# Auto-generated by scripts/deploy/azure_deploy_container.py",
...
def sync_github_actions_vars_secrets(*, repo_root: Path, deploy_env_path: Path | None, azure_client_id: str | None = None) -> None:
    script = repo_root / "scripts" / "deploy" / "gh_sync_actions_env.py"
    if not script.exists():
        print("⚠️  [warn] scripts/deploy/gh_sync_actions_env.py not found; skipping vars/secrets sync", file=sys.stderr)
        return
...
def _env_filtered_content(*, env_path: Path, prefixes: list[str], raw: bool) -> str:
    ...
    header = "\n".join(
        [
            "# Generated by scripts/deploy/azure_deploy_container.py",
