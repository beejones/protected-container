...
- **Dataclass-first**: Use dataclasses for internal data structures by default. `to_dict()` and `from_dict()` methods are permitted only at API/JSON/file boundaries and must be clearly labelled as restricted serialization utilities. Use `JSONValue` for JSON serialization payload values only; `dict[str, JSONValue]`, `Mapping[str, JSONValue]`, `list[JSONValue]`, and standalone `JSONValue` are still loose raw-payload shapes, not typed domain models. Search for existing data classes before creating new ones.
- **Forbidden loose typing in new core logic**: Do not introduce `Any`, `object`, `dict[str, object]`, `Dict[str, object]`, `Mapping[str, object]`, `MutableMapping[str, object]`, generic `dict[str, JSONValue]`, `Mapping[str, JSONValue]`, `MutableMapping[str, JSONValue]`, `list[JSONValue]`, standalone `JSONValue`, untyped `dict`, or optional-required parameters outside explicit boundary adapters. If a function needs multiple fields from a payload, create or reuse a dataclass/protocol/enum.
...
- **Boundary Typing Principle**: Normalize nullable/untyped input at API or adapter boundaries, then pass strict typed values internally. `Mapping[str, JSONValue]`/`dict[str, JSONValue]` may appear in clearly named boundary functions such as `*_from_payload`, `*_from_mapping`, `*_to_payload`, `to_dict`, and `from_dict`; do not pass them through internal helper layers as if they were typed models. A helper is not a boundary just because its name contains `payload`: if it enriches, resolves, builds, scores, selects, or applies business rules, use dataclasses/protocols/enums for that operation and keep payload copying in a separate serialization adapter. Core/public signatures should use explicit dataclasses/protocols and non-optional types when the value is required.
...
- **Changelog and versioning**: Use `.github/skills/changelog/SKILL.md` for merge-time `APP_VERSION` bumps, root `CHANGELOG.md` entries, release-note summaries, and touched-model reporting.
