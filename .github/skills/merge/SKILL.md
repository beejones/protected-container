...
The post-merge version-log command is mandatory. It records the merged git ref in `out/deploy/version_log.csv` after `git pull` has moved `main` to that ref. If the merged git ref is new, deploy logging bumps `.env` `APP_VERSION` from the newest successful version-log row unless `.env` is already ahead. If the next deploy uses the same git ref, deploy logging reuses that version instead of treating it as a new version event. A deploy of another git ref may create the next patch version; do not require arbitrary deploy refs to already exist in the CSV.
